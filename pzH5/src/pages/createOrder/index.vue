<template>
  <div class="container">
    ...
    <van-popup v-model:show="showHospital" position="bottom" :style="{ height: '30%' }">
      <!-- 在弹出窗里选择医院 -->
      <van-picker title="请选择医院" :columns="showHospColumns" @confirm="showHospOnConfirm" @cancel="showHospital=false" />
    </van-popup>

    <van-popup v-model:show="showStartTime" position="bottom" :style="{ height: '30%' }">
      <van-date-picker title="选择日期" :min-date="minDate" :max-date="maxDate" @confirm="showTimeConfirm"
...
const showTimeConfirm = (item) => {
  ...
  // 将日期字符串转换为时间戳并赋值给表单的 starttime 字段
